...
// ✅ 새 프로젝트 생성 모달 (프로젝트 이름 + 지도 선택 버튼, 기본값 카카오맵)
function openCreateModal() {
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6 w-96">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div id="mapTypeGroup" class="flex justify-between gap-3">
                            <button id="kakaoBtn" class="flex-1 px-3 py-2 border rounded-lg bg-blue-600 text-white font-medium focus:outline-none hover:bg-blue-700 transition">
                                카카오맵
                            </button>
                            <button id="vworldBtn" class="flex-1 px-3 py-2 border rounded-lg bg-slate-100 text-slate-700 font-medium focus:outline-none hover:bg-slate-200 transition">
                                VWorld
                            </button>
                        </div>
    ...
    // 기본값: 카카오맵 활성화
    const kakaoBtn = modal.querySelector('#kakaoBtn');
    const vworldBtn = modal.querySelector('#vworldBtn');
    const hiddenInput = modal.querySelector('#selectedMapType');

    kakaoBtn.addEventListener('click', () => {
        hiddenInput.value = 'kakao';
        kakaoBtn.classList.add('bg-blue-600', 'text-white');
        kakaoBtn.classList.remove('bg-slate-100', 'text-slate-700');
        vworldBtn.classList.remove('bg-blue-600', 'text-white');
        vworldBtn.classList.add('bg-slate-100', 'text-slate-700');
    });

    vworldBtn.addEventListener('click', () => {
        hiddenInput.value = 'vworld';
        vworldBtn.classList.add('bg-blue-600', 'text-white');
        vworldBtn.classList.remove('bg-slate-100', 'text-slate-700');
        kakaoBtn.classList.remove('bg-blue-600', 'text-white');
        kakaoBtn.classList.add('bg-slate-100', 'text-slate-700');
    });
